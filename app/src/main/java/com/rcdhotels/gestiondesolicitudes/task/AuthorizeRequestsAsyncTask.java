...
public class AuthorizeRequestsAsyncTask extends AsyncTask<Void, Void, Void> {
    ...
    public AuthorizeRequestsAsyncTask(ArrayList<Request> requests, Context context) {
    ...
    protected void onPostExecute (Void aVoid){
        ...
        if (UtilsClass.currentFragment.equalsIgnoreCase("Extraction"))
            new GetExtRequestAsyncTask(context, "", -1).execute();
        else
            new GetRetRequestAsyncTask(context, "", -1).execute();
        dialog.dismiss();
    }
        ...
        if (request.getTYPE() == 1){
            ...
            switch (UtilsClass.user.getRole()) {
                case "GS_AUTOR1":
                    ...
                    if (request.getTOTAL_VERPR() > authorization.getAmount()) {
                        affectedRow = authorizeRequest(0, 1, request.getIDREQUEST());
                        ...
                        affectedRow = authorizeRequest(3, 3, request.getIDREQUEST());
                ...
                case "GS_AUTOR2":
                    ...
                    if (request.getTOTAL_VERPR() > authorization.getAmount()) {
                        affectedRow = authorizeRequest(0, 2, request.getIDREQUEST());
