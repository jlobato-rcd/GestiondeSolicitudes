...
public class RetFilterDialog extends Dialog {
    ...
    public RetFilterDialog(@NonNull Context context) {
        super(context);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinnerRequestingWarehouse.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> arg0, View arg1, int position, long arg3) {
                supplierSelected = (Warehouse) spinnerRequestingWarehouse.getSelectedItem();
            }
        ...
        buttonFilter.setOnClickListener(v -> {
            ...
            switch (user.getRole()){
                ...
                case "GS_AUTOR1":
                    ...
                    new GetRetRequestAsyncTask(context, dateFrom, dateTo, status, stgeloc, movestloc, matType).execute();
                    break;
                case "GS_AUTOR2":
                case "GS_AUTOR3":
                    if(!requestingSelected.getStgeLoc().equalsIgnoreCase("ALL"))
                ...
                case "GS_PROCE":
                    ...
                    new GetRetRequestAsyncTask(context, dateFrom, dateTo, status, movestloc, stgeloc, matType).execute();
                    break;
                case "GS_REPRO":
                    if(!requestingSelected.getStgeLoc().equalsIgnoreCase("ALL"))
                        movestloc = requestingSelected.getStgeLoc();
